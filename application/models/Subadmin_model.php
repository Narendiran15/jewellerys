...
class Subadmin_model extends My_Model
{
	...
	function export_user_details($table,$fields_wanted)
	{
		...
		$query .=' FROM '.$table.' where admin_status="1" order by id desc';
   ...
   function export_transaction_list($id)
	...
	/*Ajax Based Service List*/
	public function get_ajax_subadmin_list($limit,$start,$search,$col,$dir,$model_function_name)
    {  
		...
		{
			$query = $this->db->limit($limit,$start)->order_by($col,$dir)->get(ADMIN);
			return $query; 
		}
