...
</div>-->			<input type="hidden" class="app-input-control" name="branch_id" id="branch_id" value="<?php echo $branch_info->id;?>">			<input type="hidden" class="app-input-control" name="company_id" value="<?php echo $branch_info->company_id;?>">
						...
						<table style="width: 100%;" id="ref_table">
								...
								<tbody>
										...
										<tr class="existingitem">
												...
												<td> <input type="text" class="app-input-control required contact_name" title="Please enter contact name" name="name[]"></td>

												<td> <input type="text" class="app-input-control required" title="Please enter job title" name="job_title[]"></td>

												<td> <input type="text" class="app-input-control email contact_email required" title="Please enter valid email" name="email[]"></td>
										...
										<tr class="existingitem">
												...
												<td> <input type="text" class="app-input-control dialingcode_check" name="mobile[]"  data-code="<?php echo $calling_code;?>" ></td>
										...
										<tr class="<?php if($i==2){ echo "ref_clone";}?> existingitem">
												...
												<td> <input type="text" class="app-input-control <?php if($i==1){ echo "contact_name required";}?>" title="Please enter contact name." name="name[]" value="<?php echo $contact["name"];?>"></td>

												<td> <input type="text" class="app-input-control <?php if($i==1){ echo "required";}?>" name="job_title[]" title="Please enter job title" value="<?php echo $contact["job_title"];?>"></td>

												<td> <input type="text" class="app-input-control email <?php if($i==1){ echo "contact_email required";}?>" title="Please enter valid email" name="email[]" value="<?php echo $contact["email"];?>"></td>

												<td> <input type="text" class="app-input-control" name="skype[]" value="<?php echo $contact["skype"];?>"></td>

												<td> <input type="text" class="app-input-control dialingcode_check" name="mobile[]"  data-code="<?php echo $calling_code;?>"  value="<?php echo $contact["mobile"];?>"></td>
												<?php if($i>4){?>
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12 app_input_base text-right margin-small">
		...
		<a href="javascript:void();" onclick="load_branches();" class="backbtn"><span class="arrow-icon"><img src="<?php echo base_url();?>images/site/back-icon.png"></span>Back</a>
                ...
                messages: {
                 ...
                 submitHandler: function(form) {
					...
					$.ajax(
						{
							type: "POST",
							url: baseurl+'site/landing/save_branches',
							dataType: "json",
...
$(document).on("click","#is_ho_btn",function(e){
	...
	$.post(baseurl+"site/landing/check_headoffice",{"branch_id":branch_id},function(data){
		...
		if(data['status'] == 2){
			...
			$(".address1").val(data["head_office"]["address1"]);
			$(".address2").val(data["head_office"]["address2"]);
			$(".city").val(data["head_office"]["city"]);
			$(".state").val(data["head_office"]["state"]);
			$(".zip").val(data["head_office"]["zip"]);
			$(".phone").val(data["head_office"]["phone"]);
			$(".fax").val(data["head_office"]["fax"]);			$(".contact_name").val(data["head_office"]["contact_name"]);			$(".contact_email").val(data["head_office"]["email"]);
		}
		else{
