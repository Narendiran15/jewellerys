<?php 
$data = '';
$title = '<table>
	<tr>
    	<td colspan="2" align="right">Date :</td>
		<td colspan="2" align="left">'. date('Y-m-d'). '</td>
		<td colspan="3" align="right">Title :</td>
        <td colspan="3" align="left">'.$this->config->item('site_name').' Mail Details</td>
        
	</tr>
	<tr>
    	<td colspan="11">&nbsp;</td>
	</tr>
	</table>';	

$header = $title.'<table border="1"><tr>';

$field_count = (array) $mail[0];
$field_count=count($field_count);
$i=1;
 foreach ($mail[0] as $table_field=>$field_value)
    {
 $field_count--;
 if($table_field=="id"){$header.='<th>Sno</th>';}else{
 $header .= '<th>'.ucfirst($table_field).'</th>';
 }
 if($field_count==0)
 {
 $header .= '</tr>';
 }
    } 
 foreach ($mail as $field_name=>$field_values)
		{ 
		$data .= '<tr>';
		foreach ($field_values as $field=>$field_value)
		{
		if($field=="id"){
			$data .= '<td style="text-align:left">'.$i.'</td>';
		}else{
		$data .= '<td style="text-align:left">'.$field_value.'</td>';
		}
        }
		$data .='</tr>'; $i++;
       } 
		$data =$header.$data;
		
if ( $data == "" )
{
    $data = "\nNo Record(s) Found!\n";                        
}
header("Content-type: application/octet-stream");
header("Content-Disposition: attachment; filename=".$this->config->item('site_name')."_mail_list.xls");
header("Pragma: no-cache");
header("Expires: 0");
print "$data";
?>